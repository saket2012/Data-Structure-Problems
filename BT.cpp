...
class BT
{
   ...
   public:
	node *create(int in[], int pre[], int inStrt, int inEnd)
	{
		...
		temp->left = create(in, pre, inStrt, inIndex - 1);
		temp->right = create(in, pre, inIndex + 1, inEnd);
		return temp;
	}

	node *create1(int in[], int post[], int inStrt, int inEnd)
	{
		...
		int inIndex = search(in, inStrt, inEnd, temp->data);
		temp->right = create(in, post, inIndex + 1, inEnd);
		temp->left = create(in, post, inStrt, inIndex - 1);
		return temp;
	}
